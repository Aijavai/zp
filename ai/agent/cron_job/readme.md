...
普通函数，一调用就从头跑到尾
生成器函数，跑一些遇到yield 就暂停，promise 解决后可以从暂停的地方继续跑，是async/await 的前身
生成器函数也比较困难和复杂
...
事件1 =》 事件2 =》 事件3 =》 事件4
像一条河流，事件1 到事件4 是一个连续的事件流

pnpm install rxjs 安装 RxJS

## 流式输出

- nest.js + rxjs 实现服务器端sse 接口
    - nest.js 以@Sse 装饰器模式 /ai/chat/stream
    - 本质是 设置了 Content-Type: text/event-stream
                Cache-Control: no-cache 别缓存
                Connection: keep-alive 保持连接
                Transfer-Encoding: chunked 分块传输
    - service 模块 根据langchian stream:true   llm 流式响应
    - 使用rxjs from api 将llm 流式响应转换成一个 Observable 对象
