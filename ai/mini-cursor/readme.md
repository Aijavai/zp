...
- AI Agent 如何打造 ?
  - 直接调用大模型？ 获得智能，生成代码。
    gemini 3.1 pro
  - 你上周和它聊过的消息，它是不是记不住？（bug）Memory 记忆
  - 你让他帮你访问一个网页，做一些事情，Tool 
  - 你想让他基于公司内部的私密文档做一些解答 RAG

  AI Agent = llm（大脑） + Memory（记忆） + Tool（工具使用）  + RAG(规划能力)

## Agent 是什么？
AI Agent = llm（大脑） + Memory（记忆） + Tool（工具使用）  + RAG(规划能力)

llm 本身可以思考、规划、给他用tool 拓展能力，他就可以**自动**做事情，用memory 管理记忆，
他就可以记住你要它记住的东西，还可以使用RAG 查询内部知识获取知识（context）。

这样一个知道内部知识、能思考规划、有记忆，能够帮做事情拓展后的大模型。
...
- llm 思考（thinking）），规划（plaining） aigc 生成代码
